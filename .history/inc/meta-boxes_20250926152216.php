...
function theme_doctor_meta_box_callback($post) {
    ...
    $education = get_post_meta($post->ID, 'doctor_education_json', true);
        ...
        <div id="education-list">
            ...
            if (is_array($education_data)) {
                foreach ($education_data as $index => $edu) {
                    ...
                    echo '<input type="text" name="education_year[]" value="' . esc_attr($edu['year']) . '" placeholder="' . __('Год', 'mokrenko') . '" />';
                    echo '<input type="text" name="education_desc[]" value="' . esc_attr($edu['desc']) . '" placeholder="' . __('Описание', 'mokrenko') . '" style="width: 60%;" />';
                    echo '<button type="button" class="remove-education">' . __('Удалить', 'mokrenko') . '</button>';
                    echo '</div>';
                }
            }
            ?>
        </div>
        <button type="button" id="add-education"><?php _e('Добавить образование', 'mokrenko'); ?></button>
        <textarea name="doctor_education_json" id="doctor_education_json" style="display: none;"><?php echo esc_textarea($education); ?></textarea>
    </div>
...
function theme_save_doctor_meta($post_id) {
    ...
    // Сохранение образования (JSON)
    if (isset($_POST['education_year']) && isset($_POST['education_desc'])) {
        $education = [];
        for ($i = 0; $i < count($_POST['education_year']); $i++) {
            if (!empty($_POST['education_year'][$i]) && !empty($_POST['education_desc'][$i])) {
                $education[] = [
                    'year' => sanitize_text_field($_POST['education_year'][$i]),
                    'desc' => sanitize_text_field($_POST['education_desc'][$i])
                ];
            }
        }
        update_post_meta($post_id, 'doctor_education_json', json_encode($education));
    }
